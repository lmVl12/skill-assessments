...
display_board <- function(board, score) {
  ...
  cat(" ===================================\n")
  cat(sprintf(
    " SCORE | Human: %d | Computer: %d | Ties: %d\n",
...
get_valid_input <- function(prompt_msg, valid_options) {
  while (TRUE) {
    ...
    if (length(user_input) > 0 && user_input %in% valid_options) {
      return(user_input)
    }
...
while (TRUE) {
  ...
  if (result == "human") {
    score_tracker$human <- score_tracker$human + 1
  } else if (result == "computer") {
    score_tracker$computer <- score_tracker$computer + 1
  } else {
    score_tracker$ties <- score_tracker$ties + 1
  }
