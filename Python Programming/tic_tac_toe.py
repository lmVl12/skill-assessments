...
def colorize(sym):
    ...
    if sym == "X": return f"{GREEN}{BOLD}X{RESET}"
    if sym == "O": return f"{RED}{BOLD}O{RESET}"
    ...
    while True:
        ...
        if winner_sym:
            if winner_sym == user_sym:
                score['user'] += 1
                print(f"{GREEN}🎉 YOU WON!{RESET}"); return "user"
            else:
                score['comp'] += 1
                print(f"{RED}🤖 COMPUTER WON!{RESET}"); return "comp"

        if is_draw_inevitable(board):
            score['draw'] += 1
            print(f"{YELLOW}🤝 DRAW!{RESET}"); return "draw"
        ...
        if current_turn == user_sym:
            ...
            for r in range(3):
                for c in range(3):
                    if board[r][c] == move: board[r][c] = comp_sym
